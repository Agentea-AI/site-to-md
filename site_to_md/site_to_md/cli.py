...
def main() -> Optional[int]:
    ...
    try:
        ...
        # If output_dir is specified, make it absolute and expand user path
        output_dir = None
        if args.output_dir:
            output_dir = str(Path(args.output_dir).expanduser().resolve())
        else:
            # Use default directory in user's Documents
            output_dir = str(Path.home() / "Documents" / "website-markdown")
        
        # Convert the URL
        converter = URLToMarkdownConverter(config)
        output_file = converter.convert(args.url, output_dir)
